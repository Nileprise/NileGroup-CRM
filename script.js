...
const state = {
    ...
    filters: { text: '', recruiter: '', tech: '', status: '' }, hubFilters: { text: '', recruiter: '' }, onbFilters: { text: '' }, empFilters: { text: '' },
...
const dom = {
    ...
    views: { dashboard: document.getElementById('view-dashboard'), inbox: document.getElementById('view-inbox'), candidates: document.getElementById('view-candidates'), hub: document.getElementById('view-hub'), employees: document.getElementById('view-employees'), onboarding: document.getElementById('view-onboarding'), settings: document.getElementById('view-settings'), profile: document.getElementById('view-profile'), placements: document.getElementById('view-placements') },
...
function init() {
    ...
    auth.onAuthStateChanged(async user => {
        if (user) {
            if (!user.emailVerified) { document.getElementById('verify-email-display').innerText = user.email; switchScreen('verify'); return; }
            state.user = user; const email = user.email.toLowerCase();
            try {
                const userDoc = await db.collection('users').doc(email).get();
                if (userDoc.exists) { state.userRole = userDoc.data().role || 'Employee'; state.currentUserName = userDoc.data().firstName || user.displayName || 'Unknown'; } 
                else { const knownUser = ALLOWED_USERS[email]; state.userRole = knownUser ? knownUser.role : 'Employee'; state.currentUserName = knownUser ? knownUser.name : (user.displayName || 'Unknown'); }
...
function showTableLoaders() {
    const loaderHTML = `<tr><td colspan="20" style="text-align: center; padding: 40px; color: var(--text-muted);"><i class="fa-solid fa-circle-notch fa-spin text-cyan" style="font-size: 2rem; margin-bottom: 15px;"></i><br>Connecting to secure server...</td></tr>`;
    ['table-body', 'employee-table-body', 'hub-table-body', 'placement-table-body', 'onboarding-table-body'].forEach(id => {
        if(document.getElementById(id)) document.getElementById(id).innerHTML = loaderHTML;
    });
}
...
function switchScreen(screenName) { Object.values(dom.screens).forEach(s => s.classList.remove('active')); if(dom.screens[screenName]) dom.screens[screenName].classList.add('active'); }
...
function initRealtimeListeners() {
    ...
    candRef.onSnapshot(snap => {
        ...
        state.candidates.sort((a, b) => (a.orderIndex !== undefined ? a.orderIndex : -a.createdAt) - (b.orderIndex !== undefined ? b.orderIndex : -b.createdAt));
        const currentSelectedCount = state.selection.cand.size;
        renderCandidateTable(); if (currentSelectedCount > 0) updateSelectButtons('cand');
    ...
    db.collection('employees').onSnapshot(snap => {
        ...
        state.employees.sort((a, b) => (a.orderIndex !== undefined ? a.orderIndex : -a.createdAt) - (b.orderIndex !== undefined ? b.orderIndex : -b.createdAt));
...
function loadCustomColumns() { 
    db.collection('settings').doc('table_config').onSnapshot(doc => { 
        if(doc.exists) { 
            const data = doc.data(); 
            if(data.candidates) state.customColumns.candidates = data.candidates; if(data.employees) state.customColumns.employees = data.employees; if(data.onboarding) state.customColumns.onboarding = data.onboarding; if(data.placements) state.customColumns.placements = data.placements; if(data.colOrders) state.colOrders = data.colOrders; 
...
function renderDropdowns() { 
    ['filter-recruiter', 'filter-tech'].forEach(id => { 
        ...
        const currentVal = el.value; let opts = id.includes('tech') ? state.metadata.techs.map(t => `<option value="${t}">${t}</option>`).join('') : state.metadata.recruiters.map(r => `<option value="${r.value}">${r.display}</option>`).join(''); 
        el.innerHTML = `<option value="">${id.includes('tech')?"All Tech":"All Recruiters"}</option>${opts}`; el.value = currentVal; 
    }); 
}
...
function getOldValue(collection, id, field) { const item = (state[collection] || []).find(x => x.id === id); return item ? item[field] : null; }
function pushToHistory(collection, id, field, oldVal, newVal) { historyState.undo.push({ collection, id, field, oldVal, newVal }); }
...
function renderDashboardCharts() { 
    ...
    candData.forEach(c => { 
        const r = c.recruiter ? c.recruiter.trim() : 'Unassigned'; recCounts[r] = (recCounts[r] || 0) + 1; 
        let tRaw = c.tech ? c.tech.trim() : 'Other'; if(tRaw === '') tRaw = 'Other'; const existingKey = Object.keys(techCounts).find(k => k.toLowerCase() === tRaw.toLowerCase()); const t = existingKey ? existingKey : tRaw; techCounts[t] = (techCounts[t] || 0) + 1; 
    }); 
    ...
    if (recWrapper) { const requiredWidth = Math.max(100, recLabels.length * 60); recWrapper.innerHTML = `<div class="canvas-scroll-inner" style="width: ${requiredWidth > 100 ? requiredWidth + 'px' : '100%'}"><canvas id="chart-recruiter"></canvas></div>`; }
    ...
    const techWrapper = document.querySelector('.small-chart .canvas-wrapper'); if (techWrapper) { techWrapper.innerHTML = `<div class="canvas-scroll-inner" style="width: 100%;"><canvas id="chart-tech"></canvas></div>`; }
...
function updateDashboardStats() { 
    let candData = state.candidates.filter(c => c.status !== 'Placed'); let placedData = state.placements;
    if (state.userRole === 'Employee' && state.currentUserName) { candData = candData.filter(c => c.recruiter === state.currentUserName); placedData = placedData.filter(c => c.recruiter === state.currentUserName); }
...
function restoreColumnOrder(tableId, context) {
    ...
    savedOrder.forEach((colName, desiredRelativeIdx) => {
        const desiredDOMIdx = desiredRelativeIdx + 4; const headers = Array.from(table.querySelectorAll('th')); let currentDOMIdx = -1;
...
window.initResize = function(e) {
    ...
    startX = e.pageX; startWidth = resizingTh.offsetWidth;
    resizingTh.classList.add('active'); document.body.style.cursor = 'col-resize';
    document.addEventListener('mousemove', doResize); document.addEventListener('mouseup', stopResize);
};
function doResize(e) {
    if (!resizingTh) return;
    const newWidth = startWidth + (e.pageX - startX);
    if (newWidth > 50) { resizingTh.style.width = `${newWidth}px`; resizingTh.style.minWidth = `${newWidth}px`; resizingTh.style.maxWidth = `${newWidth}px`; }
}
function stopResize() {
    if (resizingTh) { resizingTh.classList.remove('active'); resizingTh = null; }
    document.body.style.cursor = ''; document.removeEventListener('mousemove', doResize); document.removeEventListener('mouseup', stopResize);
}
...
function renderOnboardingTable() {
    ...
    // Divider applied to Last Name
    document.getElementById('onboarding-table-head').innerHTML = `<tr><th style="width:40px; text-align:center;"><div style="display:flex; flex-direction:column; gap:5px; align-items:center;"><i class="fa-solid fa-table-columns hover-primary" style="cursor:pointer;" onclick="openAddColumnModal('onboarding')" title="Add New Column"></i><i class="fa-solid fa-arrows-left-right-to-line hover-primary" style="cursor:pointer; font-size:0.8rem;" onclick="cycleAlignAll('onboarding')"></i></div></th><th><input type="checkbox" id="select-all-onb" onclick="toggleSelectAll('onb', this)" ${isAllChecked ? 'checked' : ''}></th><th>${thAlign('#', 'onboarding')}</th><th>${thAlign('First Name', 'onboarding')}</th><th class="divider-col" style="position:relative;">${thAlign('Last Name', 'onboarding')}<div class="resizer" onmousedown="initResize(event)"></div></th><th>${thAlign('Date of Birth', 'onboarding')}</th><th>${thAlign('Recruiter', 'onboarding')}</th><th>${thAlign('Mobile', 'onboarding')}</th><th>${thAlign('Status', 'onboarding')}</th><th>${thAlign('Assigned', 'onboarding')}</th><th>${thAlign('Comments', 'onboarding')}</th>${customHeaders}</tr>`;
...
function renderPlacementTable() {
    ...
    const validIds = new Set(placed.map(c => c.id)); state.selection.place.forEach(id => { if(!validIds.has(id)) state.selection.place.delete(id); });
...
function renderHubTable() {
    ...
    const validIds = new Set(activeCandidates.map(c => c.id)); state.selection.hub.forEach(id => { if(!validIds.has(id)) state.selection.hub.delete(id); });
    ...
    // Divider applied to Technology
    document.getElementById('hub-table-head').innerHTML = `<tr><th style="width:40px; text-align:center;"><div style="display:flex; flex-direction:column; gap:5px; align-items:center;"><i class="fa-solid fa-table-columns hover-primary" style="cursor:pointer;" onclick="openAddColumnModal('hub')" title="Add New Column"></i><i class="fa-solid fa-arrows-left-right-to-line hover-primary" style="cursor:pointer; font-size:0.8rem;" onclick="cycleAlignAll('hub')"></i></div></th><th style="width:40px;"><input type="checkbox" id="select-all-hub" onclick="toggleSelectAll('hub', this)" ${isAllChecked ? 'checked' : ''}></th><th style="width:50px;">${thAlign('#', 'hub')}</th><th style="width:150px;">${thAlign('Candidate Name', 'hub')}</th><th style="width:150px;">${thAlign('Recruiter', 'hub')}</th><th class="divider-col" style="width:120px; position:relative;">${thAlign('Technology', 'hub')}<div class="resizer" onmousedown="initResize(event)"></div></th><th style="text-align:center;">${thAlign('Submission', 'hub')}</th><th style="text-align:center;">${thAlign('Screenings', 'hub')}</th><th style="text-align:center;">${thAlign('Interview', 'hub')}</th><th style="text-align:right;">${thAlign('Date', 'hub')}</th></tr>`;
...
window.updateHubStats = (filterType, dateVal) => {
    ...
    const dateInput = document.getElementById('hub-date-picker'); if (dateInput && dateInput.value !== state.hub.date) { dateInput.value = state.hub.date; }
    const [year, month, day] = state.hub.date.split('-').map(Number); const d = new Date(year, month - 1, day); 
    let start, end, labelText;

    if (state.hub.filterType === 'daily') { 
        start = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 0, 0, 0, 0).getTime(); end = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 23, 59, 59, 999).getTime(); labelText = state.hub.date; 
    } 
    else if (state.hub.filterType === 'weekly') { 
        const currentDay = d.getDay(); const distanceToMonday = currentDay === 0 ? -6 : 1 - currentDay; 
        const monday = new Date(d.getFullYear(), d.getMonth(), d.getDate() + distanceToMonday); const friday = new Date(monday.getFullYear(), monday.getMonth(), monday.getDate() + 4);
        start = new Date(monday.getFullYear(), monday.getMonth(), monday.getDate(), 0, 0, 0, 0).getTime(); end = new Date(friday.getFullYear(), friday.getMonth(), friday.getDate(), 23, 59, 59, 999).getTime(); 
        ...
        start = new Date(d.getFullYear(), d.getMonth(), 1, 0, 0, 0, 0).getTime(); end = new Date(d.getFullYear(), d.getMonth() + 1, 0, 23, 59, 59, 999).getTime(); labelText = d.toLocaleString('default', { month: 'long', year: 'numeric' }); 
    }
    ...
    let subs=0, scrs=0, ints=0; let hubDataCount = state.candidates;
    if (state.userRole === 'Employee' && state.currentUserName) hubDataCount = hubDataCount.filter(c => c.recruiter === state.currentUserName);
    hubDataCount.forEach(c => { subs += (c.submissionLog||[]).filter(isInRange).length; scrs += (c.screeningLog||[]).filter(isInRange).length; ints += (c.interviewLog||[]).filter(isInRange).length; });
...
window.createNewRow = async (type) => {
    const ts = Date.now() + Math.random(); const newOrderIndex = -ts; const defaultRecruiter = state.userRole === 'Employee' ? state.currentUserName : '';
...
window.manualAddPlacement = async () => {
    const ts = Date.now() + Math.random(); let defaultDate = new Date().toISOString().split('T')[0];
    const mVal = document.getElementById('placement-month-picker')?.value; const yVal = document.getElementById('placement-year-picker')?.value;
    if (state.placementFilter === 'monthly' && mVal) defaultDate = `${mVal}-01`; else if (state.placementFilter === 'yearly' && yVal) defaultDate = `${yVal}-01-01`;
    ...
    try { await db.collection('placements').add(data); showToast("Blank placement added"); } catch (error) { showToast("Error adding placement"); } 
};

window.inlineEdit = (id, field, col, el) => { 
    if (el.querySelector('input')) return; 
    el.tabIndex = 0; el.dataset.field = field; const val = el.innerText; 
    el.innerHTML = `<input type="text" class="inline-input-active" value="${val}">`; 
    const input = el.querySelector('input'); input.focus(); input.selectionStart = input.selectionEnd = input.value.length; 
...
window.saveInline = (input, id, field, col, oldVal) => { 
    const newVal = input.value.trim(); input.parentElement.innerText = newVal; 
    if(newVal !== oldVal) { pushToHistory(col, id, field, oldVal, newVal); db.collection(col).doc(id).update({[field]: newVal}).then(() => showToast("Auto-Saved")).catch(()=>input.parentElement.innerText = oldVal); } 
};
...
window.inlineUrlEdit = (id, field, col, el) => { 
    if(el.querySelector('input')) return; 
    let oldVal = getOldValue(col, id, field) || ''; 
    let displayVal = oldVal.replace('mailto:', '').replace('https://', '').replace('http://', '');
    el.innerHTML = ''; 
    const input = document.createElement('input'); input.type = 'text'; input.placeholder = 'Paste Link or Email...'; input.className = 'url-input-active'; input.value = displayVal; 
    input.onclick = (e) => e.stopPropagation(); input.ondblclick = (e) => e.stopPropagation(); 
    
    const save = () => { 
        let newVal = input.value.trim(); 
        if (newVal) { if (newVal.includes('@') && !newVal.includes('/')) { newVal = 'mailto:' + newVal; } else { if (!newVal.startsWith('http') && !newVal.startsWith('mailto:')) newVal = 'https://' + newVal; } }
        if(newVal !== oldVal) { pushToHistory(col, id, field, oldVal, newVal); db.collection(col).doc(id).update({ [field]: newVal }).then(() => { showToast("Link Auto-Saved"); refreshViewForType(col); }); } 
        else { refreshViewForType(col); }
    }; 
    
    input.addEventListener('blur', save); input.addEventListener('keydown', (e) => { if (e.key === 'Enter') input.blur(); if (e.key === 'Escape') refreshViewForType(col); }); 
...
window.executeAddColumn = async () => { 
    ...
    state.customColumns[activeColumnContext].push({ name, key, type }); await saveAndRefreshColumns(activeColumnContext, `Column "${name}" Added`); document.getElementById('new-col-name').value = ''; openAddColumnModal(activeColumnContext); 
};
...
function refreshViewForType(type) { 
    if(type==='cand' || type==='candidates') renderCandidateTable(); else if(type==='emp' || type==='employees') renderEmployeeTable(); else if(type==='onb' || type==='onboarding') renderOnboardingTable(); else if(type==='hub') renderHubTable(); else if(type==='place' || type==='placements') renderPlacementTable(); 
}

function updateSelectButtons(type) { 
    let btn, countSpan; 
    if(type === 'cand') { btn = document.getElementById('btn-delete-selected'); countSpan = document.getElementById('selected-count'); } else if(type === 'emp') { btn = document.getElementById('btn-delete-employee'); countSpan = document.getElementById('emp-selected-count'); } else if(type === 'onb') { btn = document.getElementById('btn-delete-onboarding'); countSpan = document.getElementById('onboarding-selected-count'); } else if(type === 'place') { btn = document.getElementById('btn-delete-placement'); countSpan = document.getElementById('place-selected-count'); } else if(type === 'hub') { btn = document.getElementById('btn-delete-hub'); countSpan = document.getElementById('hub-selected-count'); } 
    if (!btn) return; 
    if (state.selection[type] && state.selection[type].size > 0 && state.userRole !== 'Employee') { btn.style.display = 'inline-flex'; btn.style.opacity = '1'; if(countSpan) countSpan.innerText = state.selection[type].size; } else { btn.style.display = 'none'; if(countSpan) countSpan.innerText = '0'; } 
}
...
function loadGoogleScripts() { 
    ...
    s2.onload = () => { state.gmail.tokenClient = google.accounts.oauth2.initTokenClient({ client_id: G_CLIENT_ID, scope: G_SCOPES, callback: (resp) => { if(resp.error) return; updateGmailUI(true); renderGmailList('INBOX'); fetchGmailLabels(); startMailboxSync(); } }); state.gmail.gisInited = true; checkGmailAuth(); }; 
    document.body.appendChild(s2); 
}

function checkGmailAuth() { 
    if (state.gmail.gapiInited && state.gmail.gisInited) {
        if (gapi.client.getToken()) { updateGmailUI(true); fetchGmailLabels(); startMailboxSync(); setInterval(startMailboxSync, 5 * 60 * 1000); } 
...
function parseMessageBody(payload) {
    const decodeBase64Utf8 = (base64Str) => {
        ...
        catch (e) { console.warn("Email decoding failed.", e); return "(Encoding Error)"; }
    };
    let bodyText = ''; let bodyHtml = '';
    if (payload.body && payload.body.data) { const decodedString = decodeBase64Utf8(payload.body.data); if (payload.mimeType === 'text/html') { bodyHtml = decodedString; } else if (payload.mimeType === 'text/plain') { bodyText = decodedString; } }
    let attachments = [];
    if (payload.parts) {
        const parsedParts = payload.parts.reduce((acc, part) => {
            if (part.filename && part.filename.length > 0) { acc.attachments.push({ filename: part.filename, mimeType: part.mimeType, size: part.body.size, attachmentId: part.body.attachmentId }); } 
            else { const nestedResult = parseMessageBody(part); acc.text += nestedResult.text; acc.html += nestedResult.html; acc.attachments = acc.attachments.concat(nestedResult.attachments); }
            return acc;
...
window.renderLabels = () => { 
    const container = document.getElementById('dynamic-labels-container'); if(!container) return; container.innerHTML = ""; if(document.getElementById('manage-indicator')) document.getElementById('manage-indicator').style.display = 'none'; 
    state.labels.forEach((l, index) => { 
        const div = document.createElement('div'); div.className = 'label-item'; const isSub = l.name.includes('/'); const displayName = isSub ? l.name.split('/').pop() : l.name; const indent = isSub ? 'padding-left: 20px;' : ''; 
...
window.exportData = () => {
    ...
    dataToExport.forEach(c => { rows.push([escapeCsv(c.id), escapeCsv(c.first), escapeCsv(c.last), escapeCsv(c.mobile), escapeCsv(c.wa), escapeCsv(c.tech), escapeCsv(c.recruiter), escapeCsv(c.status), escapeCsv(c.assigned), escapeCsv(c.comments)]); });
    const link = document.createElement("a"); link.setAttribute("href", encodeURI("data:text/csv;charset=utf-8," + rows.map(e => e.join(",")).join("\n"))); link.setAttribute("download", `Nileprise_Candidates_${new Date().toISOString().split('T')[0]}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link); showToast("Exported successfully");
};
...
window.resetSystem = async () => {
    ...
    if(confirm("CRITICAL WARNING: This will permanently delete ALL candidates from the cloud database. This CANNOT be undone. Continue?")) {
        ...
        if (confirmText === 'DELETE') {
            ...
            try { const batch = db.batch(); state.candidates.forEach(c => { batch.delete(db.collection('candidates').doc(c.id)); }); await batch.commit(); showToast("System reset successfully."); } 
...
window.manualSync = async () => {
    const icon = document.getElementById('sync-icon'); const headerText = document.getElementById('header-updated');
    if(icon) icon.classList.add('fa-spin'); if(headerText) headerText.innerText = 'Syncing...'; showToast("Pulling fresh data from server...");
    try {
        await db.enableNetwork();
        showTableLoaders();
        const snap = await db.collection('candidates').get({ source: 'server' }); state.candidates = []; snap.forEach(doc => state.candidates.push({ id: doc.id, ...doc.data() }));
        state.candidates.sort((a, b) => (a.orderIndex !== undefined ? a.orderIndex : -a.createdAt) - (b.orderIndex !== undefined ? b.orderIndex : -b.createdAt));
        renderCandidateTable(); renderHubTable(); updateDashboardStats(); renderDashboardCharts();
        setTimeout(() => { if(icon) icon.classList.remove('fa-spin'); if(headerText) headerText.innerText = 'Synced Just Now'; showToast("Data is fully synced across devices!"); }, 500);
    } catch (error) {
        console.error("Hard sync failed:", error); if(icon) icon.classList.remove('fa-spin'); if(headerText) headerText.innerText = 'Sync Failed'; showToast("Network error. Could not reach server.");
    }
};
...
function setupEventListeners() {
    document.querySelectorAll('.nav-item[data-target]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (btn.classList.contains('locked')) { showToast("Access Restricted: Manager clearance required."); return; }
            document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active')); btn.classList.add('active');
            document.querySelectorAll('.content-view').forEach(v => v.classList.remove('active')); const targetId = btn.dataset.target; const targetView = document.getElementById(targetId); if(targetView) targetView.classList.add('active');
            const titleEl = document.getElementById('page-title'); if(titleEl) { const icon = btn.querySelector('i, .material-icons')?.outerHTML || ''; const text = btn.querySelector('span:not(.material-icons)')?.innerText || btn.innerText; titleEl.innerHTML = `${icon} ${text}`; }
            if(window.innerWidth <= 900) { document.getElementById('sidebar').classList.remove('mobile-open'); document.getElementById('sidebar-overlay').classList.remove('active'); }
    ...
    if(logoutBtn) { logoutBtn.addEventListener('click', () => { if(confirm("Are you sure you want to log out?")) { auth.signOut(); } }); }
    ...
    document.querySelectorAll('#view-candidates .btn-toggle').forEach(btn => { btn.addEventListener('click', (e) => { document.querySelectorAll('#view-candidates .btn-toggle').forEach(b => b.classList.remove('active')); e.target.classList.add('active'); state.filters.status = e.target.dataset.status; renderCandidateTable(); }); });
    document.getElementById('btn-reset-filters')?.addEventListener('click', () => { if(document.getElementById('search-input')) document.getElementById('search-input').value = ''; if(document.getElementById('filter-recruiter')) document.getElementById('filter-recruiter').value = ''; if(document.getElementById('filter-tech')) document.getElementById('filter-tech').value = ''; state.filters = { text: '', recruiter: '', tech: '', status: '' }; document.querySelectorAll('#view-candidates .btn-toggle').forEach(b => b.classList.remove('active')); const defaultToggle = document.querySelector('#view-candidates .btn-toggle[data-status=""]'); if(defaultToggle) defaultToggle.classList.add('active'); renderCandidateTable(); });
}
...
window.handleDragOver = (e) => { e.preventDefault(); const row = e.target.closest('tr'); if(row) { document.querySelectorAll('tr').forEach(tr => tr.style.borderTop = ''); row.style.borderTop = '2px solid var(--primary)'; } };
window.handleDrop = async (e, collection) => {
    ...
    try {
        ...
        await db.collection(collection).doc(draggedId).update({ orderIndex: newOrderIndex }); showToast("Row reordered");
...
function updateUserProfile(user, knownUser) {
    ...
    db.collection('users').doc(email).get().then(doc => {
        if(doc.exists) {
            const data = doc.data();
            if(data.firstName) document.getElementById('prof-first').value = data.firstName; if(data.lastName) document.getElementById('prof-last').value = data.lastName; if(data.dob) document.getElementById('prof-dob').value = data.dob; if(data.workMobile) document.getElementById('prof-work-mobile').value = data.workMobile; if(data.personalMobile) document.getElementById('prof-personal-mobile').value = data.personalMobile; if(data.personalEmail) document.getElementById('prof-personal-email').value = data.personalEmail;
            if(data.photoURL) { const img = document.getElementById('profile-main-img'); img.src = data.photoURL; img.style.display = 'block'; document.getElementById('profile-main-icon').style.display = 'none'; document.getElementById('btn-delete-photo').style.display = 'inline-flex'; }
        }
    });
}
...
window.saveProfileData = async () => {
    ...
    const profileData = { firstName: document.getElementById('prof-first').value, lastName: document.getElementById('prof-last').value, dob: document.getElementById('prof-dob').value, workMobile: document.getElementById('prof-work-mobile').value, personalMobile: document.getElementById('prof-personal-mobile').value, personalEmail: document.getElementById('prof-personal-email').value };
    try { await db.collection('users').doc(email).set(profileData, { merge: true }); showToast("Profile Updated Successfully"); } catch(err) { showToast("Error updating profile"); console.error(err); }
};
...
window.handlePhotoUpload = async (input) => {
    ...
    try {
        const ref = storage.ref(`profiles/${email}_${Date.now()}`); await ref.put(file); const url = await ref.getDownloadURL();
        await db.collection('users').doc(email).set({ photoURL: url }, { merge: true });
        document.getElementById('profile-main-img').src = url; document.getElementById('profile-main-img').style.display = 'block'; document.getElementById('profile-main-icon').style.display = 'none'; document.getElementById('btn-delete-photo').style.display = 'inline-flex';
        showToast("Photo uploaded");
    } catch(err) { showToast("Photo upload failed"); console.error(err); } finally { if(loadingEl) loadingEl.style.display = 'none'; }
};

window.deleteProfilePhoto = async () => {
    if(!state.user || !confirm("Remove profile photo?")) return;
    try { await db.collection('users').doc(state.user.email).update({ photoURL: firebase.firestore.FieldValue.delete() }); document.getElementById('profile-main-img').style.display = 'none'; document.getElementById('profile-main-img').src = ''; document.getElementById('profile-main-icon').style.display = 'flex'; document.getElementById('btn-delete-photo').style.display = 'none'; showToast("Photo removed"); } catch(err) { showToast("Failed to remove photo"); }
};
