...
/* ---------- Modal Close Helpers (for HTML modals) ---------- */
window.closeCandidateModal = () => { document.getElementById('add-candidate-modal').style.display = 'none'; };
...
/* ---------- Open Modal Helpers (show modal overlay) ---------- */
window.openCandidateModal = () => { document.getElementById('add-candidate-modal').style.display = 'flex'; };
window.openEmployeeModal = () => { document.getElementById('add-employee-modal').style.display = 'flex'; };
window.openOnboardingModal = () => { document.getElementById('add-onboarding-modal').style.display = 'flex'; };
window.openPlacementModal = () => { document.getElementById('add-placement-modal').style.display = 'flex'; };
window.openHubNoteModal = () => { document.getElementById('add-hub-note-modal').style.display = 'flex'; };


/* ---------- Form Submit Handlers (for HTML modals) ---------- */
document.addEventListener('DOMContentLoaded', () => {
    const candidateForm = document.getElementById('candidate-form');
    if(candidateForm) candidateForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        const data = {
            first: document.getElementById('cand-first').value,
            last: document.getElementById('cand-last').value,
            mobile: document.getElementById('cand-mobile').value,
            wa: document.getElementById('cand-wa').value,
            tech: document.getElementById('cand-tech').value,
            recruiter: document.getElementById('cand-recruiter').value,
            status: document.getElementById('cand-status').value,
            comments: document.getElementById('cand-comments').value,
            assigned: new Date().toISOString().split('T')[0],
            orderIndex: -(Date.now() + Math.random()),
            createdAt: Date.now()
        };
        try { await db.collection('candidates').add(data); showToast("Candidate Added"); closeCandidateModal(); candidateForm.reset(); }
        catch(err) { showToast("Error adding candidate"); console.error(err); }
    });
    ...
    if(employeeForm) employeeForm.addEventListener('submit', async (e) => {
        ...
        const data = {
            ...
            workMobile: document.getElementById('emp-work-mobile').value,
            personalMobile: document.getElementById('emp-personal-mobile').value,
            officialEmail: document.getElementById('emp-official-email').value,
            personalEmail: document.getElementById('emp-personal-email').value,
            dob: document.getElementById('emp-dob').value,
            orderIndex: -(Date.now() + Math.random()),
            createdAt: Date.now()
        };
        try { await db.collection('employees').add(data); showToast("Employee Added"); closeEmployeeModal(); employeeForm.reset(); }
        catch(err) { showToast("Error adding employee"); console.error(err); }
    });
    ...
    if(onboardingForm) onboardingForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        const data = {
            first: document.getElementById('onb-first').value,
            last: document.getElementById('onb-last').value,
            mobile: document.getElementById('onb-mobile').value,
            recruiter: document.getElementById('onb-recruiter').value,
            status: document.getElementById('onb-status').value,
            dob: document.getElementById('onb-dob').value,
            comments: document.getElementById('onb-comments').value,
            orderIndex: -(Date.now() + Math.random()),
            createdAt: Date.now()
        };
        try { await db.collection('onboarding').add(data); showToast("Record Added"); closeOnboardingModal(); onboardingForm.reset(); }
        catch(err) { showToast("Error adding record"); console.error(err); }
    });

    const placementForm = document.getElementById('placement-form');
    if(placementForm) placementForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        const data = {
            first: document.getElementById('place-first').value,
            last: document.getElementById('place-last').value,
            tech: document.getElementById('place-tech').value,
            location: document.getElementById('place-location').value,
            contract: document.getElementById('place-contract').value,
            recruiter: document.getElementById('place-recruiter').value,
            assigned: document.getElementById('place-date').value || new Date().toISOString().split('T')[0],
            actions: document.getElementById('place-actions') ? document.getElementById('place-actions').value : '',
            orderIndex: -(Date.now() + Math.random()),
            createdAt: Date.now()
        };
        try { await db.collection('placements').add(data); showToast("Placement Added"); closePlacementModal(); placementForm.reset(); }
        catch(err) { showToast("Error adding placement"); console.error(err); }
    });

    const hubNoteForm = document.getElementById('hub-note-form');
    if(hubNoteForm) hubNoteForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        const candidateId = document.getElementById('hub-note-candidate').value;
        const type = document.getElementById('hub-note-type').value;
        const date = document.getElementById('hub-note-date').value;
        const subject = document.getElementById('hub-note-subject').value;
        if(!candidateId || !type || !date || !subject) return showToast("All fields required");

        const cand = state.candidates.find(c => c.id === candidateId);
        if(!cand) return showToast("Candidate not found");

        const logKey = type + 'Log';
        let logs = cand[logKey] || [];
        logs.push({ date, note: subject, recruiter: state.currentUserName, timestamp: Date.now() });

        try { await db.collection('candidates').doc(candidateId).update({ [logKey]: logs }); showToast("Hub Entry Added"); closeHubNoteModal(); hubNoteForm.reset(); }
        catch(err) { showToast("Error adding hub entry"); console.error(err); }
    });
});
