...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initial "Add" Buttons (Fallback listeners)
...
function setupUIListeners() {
    document.querySelectorAll('.nav-item').forEach(btn => {
        btn.addEventListener('click', (e) => {
            // Skip toggle buttons or options
...
window.renderLabels = () => {
    ...
    if(document.getElementById('manage-indicator')) 
    ...
    state.labels.forEach((l, index) => {
        ...
        div.innerHTML = `
            <div class="label-left" style="${indent}" onclick="renderGmailList('${l.name}')">
...
window.updateLabelColor = (index, color) => {
...
window.triggerSubLabel = (index) => {
    ...
    if(subName && subName.trim()) {
        ...
        if (state.labels.some(l => l.name.toLowerCase() === fullName.toLowerCase())) { alert("Label exists!"); return; }
...
window.createLabel = () => {
    ...
    renderLabels(); closeCreateLabelModal();
};
...
window.renderGmailList = async (label = 'Inbox', navElement = null) => {
    ...
    try {
        ...
        if (qInput && qInput.value && document.activeElement === qInput) request.q = qInput.value;
        else if (['Inbox','Sent','Starred','Important','Trash','Spam'].includes(label)) request.labelIds = [label.toUpperCase()];
...
window.openGmailDetail = async (id) => {
    ...
    try {
        ...
        const findBody = (parts) => {
            ...
            for(let part of parts) { if(part.parts) { const res = findBody(part.parts); if(res) return res; } }
            return null;
        }
...
window.toggleCategories = () => {
    ...
    if (sub.style.display === 'none') sub.style.display = 'block'; else sub.style.display = 'none';
};
window.toggleMore = () => {
...
window.sendCrmEmail = async () => {
    ...
    try {
        ...
        if(candidate) {
            let logs = candidate.submissionLog || [];
            logs.push({ date: new Date().toISOString().split('T')[0], subject: subject, type: 'Outbound Email', tech: candidate.tech||'General', recruiter: state.currentUserName, timestamp: Date.now() });
            await db.collection('candidates').doc(candidate.id).update({ submissionLog: logs });
...
function loadCustomColumns() {
    db.collection('settings').doc('table_config').onSnapshot(doc => {
        if(doc.exists) {
            ...
            renderCandidateTable(); renderEmployeeTable(); renderOnboardingTable();
        }
    });
}
...
function renderDashboardCharts() {
    ...
    state.candidates.forEach(c => {
        const r = c.recruiter || 'Unknown'; recCounts[r] = (recCounts[r] || 0) + 1;
        const t = c.tech || 'Other'; techCounts[t] = (techCounts[t] || 0) + 1;
    });
    ...
    if (ctxRec) {
        ...
        recChartInstance = new Chart(ctxRec, {
            type: 'bar',
            data: { labels: recLabels, datasets: [{ label: 'Candidates', data: recData, backgroundColor: 'rgba(6, 182, 212, 0.6)', borderColor: '#06b6d4', borderWidth: 1, borderRadius: 4 }] },
    ...
    if (ctxTech) {
        ...
        techChartInstance = new Chart(ctxTech, {
            type: 'doughnut',
            data: { labels: techLabels, datasets: [{ data: techData, backgroundColor: ['rgba(6,182,212,0.7)', 'rgba(245,158,11,0.7)', 'rgba(139,92,246,0.7)', 'rgba(34,197,94,0.7)', 'rgba(239,68,68,0.7)'], borderWidth: 2 }] },
...
/* ==========================================================================
   10. TABLE RENDERERS (Insert Row & Column Logic)
   ========================================================================== */
function renderCandidateTable() {
    ...
    thead.innerHTML = `<tr>
        <th style="width:40px; text-align:center; cursor:pointer;" onclick="createNewRow('candidates')" title="Insert New Row">
            <i class="fa-solid fa-plus text-success" style="font-size: 1.1rem; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'"></i>
        </th>
        <th><input type="checkbox" id="select-all-cand" onclick="toggleSelectAll('cand', this)"></th>
        <th># <i class="fa-solid fa-table-columns header-add-btn" onclick="openAddColumnModal('candidates')" title="Add New Column"></i></th>
        <th>First Name</th><th>Last Name</th><th>Mobile</th><th>WhatsApp</th><th>Tech</th><th>Recruiter</th>
        <th style="width: 140px;">Status / Actions</th>
        <th>Assigned</th><th>Gmail</th><th>LinkedIn</th><th>Resume</th><th>Track</th><th>Comments</th>
        ${customHeaders}
    </tr>`;
    ...
    tbody.innerHTML = filtered.map((c, i) => {
        ...
        return `<tr class="${rowClass}" draggable="true" ondragstart="handleDragStart(event)" ondragover="handleDragOver(event)" ondrop="handleDrop(event)">
            ...
            <td>${i+1}</td>
            <td id="fname-${c.id}" onclick="inlineEdit('${c.id}', 'first', 'candidates', this)">${c.first}</td>
            ...
            <td style="overflow:visible;">
                <div class="action-dropdown-container">
                    ...
                    <div id="menu-${c.id}" class="custom-dropdown-menu">
                        <div class="dropdown-option" onclick="updateStatusAndClose('${c.id}', 'Active')"><span class="dot-red"></span> Set Active</div>
                        <div class="dropdown-option" onclick="updateStatusAndClose('${c.id}', 'Inactive')"><span class="dot-green"></span> Set Inactive</div>
                        <div class="dropdown-option" onclick="triggerRowEdit('${c.id}')"><i class="fa-solid fa-pen"></i> Edit</div>
                        <div class="dropdown-option delete-opt" onclick="deleteSingleCandidate('${c.id}')"><i class="fa-solid fa-trash"></i> Remove</div>
                    </div>
                </div>
            </td>
...
function renderEmployeeTable() {
    ...
    document.getElementById('employee-table-head').innerHTML = `<tr>
        <th style="width:40px; text-align:center; cursor:pointer;" onclick="createNewRow('employees')" title="Insert New Employee">
            <i class="fa-solid fa-plus text-success" style="font-size: 1.1rem; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'"></i>
        </th>
        <th><input type="checkbox" id="select-all-emp" onclick="toggleSelectAll('emp', this)"></th>
        <th># <i class="fa-solid fa-table-columns header-add-btn" onclick="openAddColumnModal('employees')" title="Add New Column"></i></th>
        <th>First Name</th><th>Last Name</th><th>Date of Birth</th><th>Designation</th><th>Work Mobile</th><th>Personal Mobile</th><th>Official Email</th><th>Personal Email</th>
        ${customHeaders}
    </tr>`;
...
function renderOnboardingTable() {
    ...
    document.getElementById('onboarding-table-head').innerHTML = `<tr>
        <th style="width:40px; text-align:center; cursor:pointer;" onclick="createNewRow('onboarding')" title="Insert New Row">
            <i class="fa-solid fa-plus text-success" style="font-size: 1.1rem; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'"></i>
        </th>
        <th><input type="checkbox" id="select-all-onb" onclick="toggleSelectAll('onb', this)"></th>
        <th># <i class="fa-solid fa-table-columns header-add-btn" onclick="openAddColumnModal('onboarding')" title="Add New Column"></i></th>
        <th>First Name</th><th>Last Name</th><th>Date of Birth</th><th>Recruiter</th><th>Mobile</th><th>Status</th><th>Assigned</th><th>Comments</th>
        ${customHeaders}
    </tr>`;
...
window.renderPlacementTable = () => {
    ...
    if(thead) thead.innerHTML = `<tr>
        <th style="width:40px; text-align:center; cursor:pointer;" onclick="manualAddPlacement()" title="Insert New Placement">
            <i class="fa-solid fa-plus text-success" style="font-size: 1.1rem; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'"></i>
        </th>
        <th style="width:40px;"><input type="checkbox" id="select-all-place" onclick="toggleSelectAll('place', this)"></th>
        <th style="width:60px;"># <i class="fa-solid fa-table-columns header-add-btn" onclick="openAddColumnModal('placements')" title="Add New Column"></i></th>
...
/* ========================================================
   11. DATA MANIPULATION (Drag, Select, Add Column)
   ======================================================== */
...
window.executeAddColumn = async () => {
    ...
    try {
        ...
        if(activeColumnContext === 'candidates') renderCandidateTable();
        else if(activeColumnContext === 'employees') renderEmployeeTable();
        else if(activeColumnContext === 'onboarding') renderOnboardingTable();
        else if(activeColumnContext === 'placements') renderPlacementTable();
...
// Custom Status Menu Helper Functions
...
function renderHubTable() {
    ...
    document.getElementById('hub-table-head').innerHTML = `<tr><th style="width:30px;"><i class="fa-solid fa-arrows-up-down-left-right"></i></th><th style="width:40px;"><input type="checkbox" id="select-all-hub" onclick="toggleSelectAll('hub', this)"></th><th style="width:60px;"># <i class="fa-solid fa-circle-plus header-add-btn" onclick="createNewRow('candidates')" title="Quick Add"></i></th><th style="width:150px;">Candidate Name</th><th style="width:150px;">Recruiter</th><th style="width:120px;">Technology</th><th style="text-align:center;">Submission</th><th style="text-align:center;">Screenings</th><th style="text-align:center;">Interview</th><th style="text-align:right;">Date</th></tr>`;
